...
that namespace's codec, retry policy and error typing, so "as openai" in its
column means the same code path, not a copy.
...
| HTTP methods | POST only | POST only | **GET, POST, DELETE** | POST only |
